...
def tshock_analyze_all_channels(df, channels, amb, amb_errors, tc_channel_names, upper_threshold, lower_threshold, tolerance, rate_adjustment, date_format, file_extension, test_name):
    ...
    for channel in channels:
        ...
        if channel != amb:
            ...
            result_each_cycle, df_summary_tc, n_reach_summary, content_instruction = single_channel_analysis(df, channel, amb, ambient, upper_threshold, lower_threshold, date_format, cycle_amount)
...
def ambient_analysis(df, channels, amb, upper_threshold, lower_threshold, date_format):
    ...
    ambient = get_amb_key_points(df_chan_Ambient, channels[0], df_chan_Ambient_loc, upper_threshold, lower_threshold)
...
def single_channel_analysis(df, channel, amb, ambient, upper_threshold, lower_threshold, date_format, cycle_amount):
    ...
    key_point_cycle, cycle_ls, n_reach, n_reach_cycle = get_keypoints_for_each_cycle(channel, ambient, df_chan, upper_threshold, lower_threshold)
    ...
    if len(consecutive_cycle) == 1 and len(consecutive_cycle[0]) >= cycle_amount - 1:
        ## Do analysis for channel

        df_chan_channel_loc = get_points_above_and_below_thresholds(df_chan, channel, upper_threshold, lower_threshold)
        selected_channel = get_amb_key_points(df_chan, channel, df_chan_channel_loc, upper_threshold, lower_threshold)

        selected_channel = calculate_ramp_stats(channel, selected_channel, df_chan_channel_loc, date_format)
        ...
        for k in range(len(consecutive_cycle)):
            ...
            for i in(consecutive_cycle[k]):
                for j in range(len(key_point_cycle)):
                    key_point_cycle[j] = key_point_cycle[j].sort_values(['Sweep_screen']).reset_index(drop=True)
